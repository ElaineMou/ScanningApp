...
public class TangoInitializationHelper {
    ...
    public static final int loadTangoSharedLibrary() {
        ...
        Log.i("TangoInitializHelper", "basePath: " + basePath);

        try {
            System.load(basePath + "arm64-v8a/libtango_client_api.so");
            loadedSoId = ARCH_ARM64;
            Log.i("TangoInitializHelper", "Success! Using arm64-v8a/libtango_client_api.");
        ...
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i("TangoInitializHelper", "Success! Using armeabi-v7a/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i("TangoInitializHelper", "Success! Using x86_64/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                loadedSoId = ARCH_X86;
                Log.i("TangoInitializHelper", "Success! Using x86/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i("TangoInitializHelper", "Success! Using default/libtango_client_api.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        if (loadedSoId < ARCH_DEFAULT) {
            try {
                ...
                Log.i("TangoInitializHelper", "Falling back to libtango_client_api.so symlink.");
            } catch (UnsatisfiedLinkError e) {
            }
        }
        return loadedSoId;
